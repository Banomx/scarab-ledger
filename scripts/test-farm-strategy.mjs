...
assert.deepEqual(clean, { name: "Harvest loop", scarabs: ["A", "A", "B", "C", "D"] });
...
assert.equal(computed.members.length, 5, "duplicate scarabs occupy separate map-device slots");
assert.equal(computed.total, 60);
...
const missing = computeFarmStrategy({ name: "Thin league", scarabs: ["A", "Gone"] }, [
  { name: "A", chaosValue: 10 },
]);
assert.deepEqual(missing.missing, ["Gone"]);
