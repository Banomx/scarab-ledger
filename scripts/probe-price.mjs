/* Find which poe.ninja endpoint carries a given item.
   ...
   Usage:
     node scripts/probe-price.mjs "Orb of Dominance"
     node scripts/probe-price.mjs "Orb of Dominance" --league Allflame
     node scripts/probe-price.mjs --counts          # just the per-endpoint totals
...
if (!needle && !countsOnly) {
  console.error('Usage: node scripts/probe-price.mjs "Item Name" [--league Allflame] [--counts]');
  process.exit(1);
}
